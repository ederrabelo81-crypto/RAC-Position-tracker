...
# 2. Python: Magalu + Google Shopping + Amazon + Leroy + Dealers
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Python: magalu google_shopping amazon leroy dealers (1 página)..." >> "$LOG"
python main.py \
...
python scripts/daily_status_check.py --turno Fechamento >> "$LOG" 2>&1
DAILY_STATUS_EXIT=$?
echo "[$(date '+%Y-%m-%d %H:%M:%S')] Validação concluída (exit=$DAILY_STATUS_EXIT)" >> "$LOG"

echo "[$(date '+%Y-%m-%d %H:%M:%S')] === Coleta noite concluída (python=$PYTHON_EXIT, status=$DAILY_STATUS_EXIT) ===" >> "$LOG"
exit $EXIT_CODE
