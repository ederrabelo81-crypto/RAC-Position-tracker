...
# Python  : amazon + leroy + dealers | prioridade: alta | páginas: 1
# Node.js : magalu                   | páginas: 1
...
(
    ...
    node_modules/.bin/ts-node src/index.ts --platforms magalu --pages 1 \
        >> "$LOG" 2>&1
...
if [ $EXIT_PYTHON -ne 0 ] || [ $EXIT_NODE -ne 0 ]; then
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] === Coleta noite LOCAL concluída com erros (python=$EXIT_PYTHON node=$EXIT_NODE) ===" >> "$LOG"
    exit 1
fi
