...
| 7 | 🟡 **Retenção/arquivamento** — PARCIAL (07/2026). Política ativa em `scripts/retention_cleanup.sql` + [`docs/DB_RETENTION.md`](DB_RETENTION.md): mantém 100% do `Diário` + 30d de intra-dia (Manhã/Tarde); coletas 90d. 1º run cortou o banco de 3,09→1,44 GB sem perder série de preço diário. FALTA: sumarizar `Diário` >12 meses em agregado semanal | Custo Supabase + velocidade | Baixo |
